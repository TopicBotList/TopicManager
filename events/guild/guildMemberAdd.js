...
module.exports.run = async (client, member, args) => {
  try {
    ...
    const systemChannel = member.guild.channels.cache.get(
      client.channelsList.systemChannel,
    );
    if (client.config.guilds.main.includes(member.guild.id)) {
      if (!member.user.bot) {
        const userJoin = new EmbedBuilder()
          ...
          .setDescription(
            `Welcome ${member.user.tag} to ${member.guild.name}. Please make sure you check out the <#${client.channelsList.rulesChannel}> and get some roles here <#${client.channelsList.rolesChannel}>!`,
          )
