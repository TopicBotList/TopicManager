  ...
  try {
    ...
    if (command) {
      ...
      if (
        ...
        !message.member.permissions.has(command.help.permissions || [])
      ) {
        const disabledCmds = new EmbedBuilder()
          .setDescription(
            `You need \`${command.help.permissions.join(", ")}\` permissions to execute ${command.help.name}.`,
          )
          .setColor(Colors.Red)
          .setFooter({
            text: client.footer,
      ...
      if (
        command.help.staff === true &&
        !client.config.staff.includes(message.author.id)
      ) {
        const notAdmin = new EmbedBuilder()
